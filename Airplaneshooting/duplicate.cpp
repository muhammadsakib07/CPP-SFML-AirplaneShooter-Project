...
int main(){
    ...
    //Assets
    sf::Font font;
    if (!font.loadFromFile("D:/PROJECT150/airplaneshooting/assets/fonts/arcade.ttf")) {
        std::cerr << "Failed to load font\n"; return -1;
    }

    // Assets: Font already loaded
    ...
    backButton.setFillColor(sf::Color(255, 255, 255, 100));


    sf::Texture bgTex;
    if (!bgTex.loadFromFile("D:/PROJECT150/airplaneshooting/assets/images/bg.png")) {
    ...
    sf::Texture enemyTexture;
        if (!enemyTexture.loadFromFile("D:/PROJECT150/airplaneshooting/assets/images/enemy7.png")) {
            std::cerr << "Failed to load enemy image!\n"; return -1;
        }

        sf::Texture fireTexture;
        if (!fireTexture.loadFromFile("D:/PROJECT150/airplaneshooting/assets/images/fire.png")) {
            std::cerr << "Failed to load fire bullet image!\n";
            return -1;
        }

    sf::Music bgMusic;
    if (!bgMusic.openFromFile("D:/PROJECT150/airplaneshooting/assets/sounds/intro.wav")) std::cerr << "Failed to load music\n";
    else { bgMusic.setLoop(true); bgMusic.play(); }

    sf::SoundBuffer selectBuf;
    sf::Sound selectSound;
    if (selectBuf.loadFromFile("D:/PROJECT150/airplaneshooting/assets/sounds/laser.wav")) selectSound.setBuffer(selectBuf);
    ...
    if (!playerTexture.loadFromFile("D:/PROJECT150/airplaneshooting/assets/images/plane0.png")) {
    ...
    // 🔹 Mouse hover এ menu highlight
    if (state == AppState::MENU && event.type == sf::Event::MouseMoved) {
        for (size_t i = 0; i < menuTexts.size(); i++) {
            if (menuTexts[i].getGlobalBounds().contains(
                (float)event.mouseMove.x, (float)event.mouseMove.y)) {
                selected = i; // hover করলে menu change হবে
            }
        }
    }

    // 🔹 Mouse click এ menu select
    if (state == AppState::MENU && event.type == sf::Event::MouseButtonPressed) {
    ...
    // 🔹 Mouse click এ bullet fire (গেম চলার সময়)
    if (state == AppState::PLAY && event.type == sf::Event::MouseButtonPressed) {
        if (event.mouseButton.button == sf::Mouse::Left) {
           sf::Vector2f bulletPos;
        bulletPos.x = playerSprite.getPosition().x + playerSprite.getGlobalBounds().width / 2 - fireTexture.getSize().x / 2.f;
        bulletPos.y = playerSprite.getPosition().y - fireTexture.getSize().y; // ঠিক player এর উপরে
    ...
    if ((state == AppState::HIGHSCORE || state == AppState::THEMES) &&
